...
// --- SYNC PROFILE PHOTO TO SERVER ---
  const handlePhotoUpload = async (file) => {
    ...
    reader.onloadend = async () => {
      ...
      setProfilePhoto(base64String); // Instant UI preview
  ...
  const handleRemovePhoto = async () => {
    ...
    try {
      ...
      console.error("Failed to remove photo on server:", err);
    }
  };
  ...
  const fetchUserData = async () => {
    ...
    const { data: profile } = await supabase.from('profiles').select('account_status, legal_name, profile_photo').eq('id', user.id).single();
    if (profile) {
      ...
      <div className="min-h-screen bg-[#e8eef3] font-sans md:pb-10 print:hidden text-gray-800 flex flex-col">
        ...
        <div className="max-w-[1400px] w-full mx-auto bg-[#00426b] md:rounded-b-xl flex flex-col md:flex-row shadow-2xl relative z-10 flex-1">
          ...
          <main className="flex-1 p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 bg-[#00426b]">
            ...
            {/* DYNAMIC RIGHT COLUMN: ONLY Renders when viewing checking or savings! */}
            {['checking', 'savings'].includes(activeTab) && (
              <div className="lg:col-span-5 flex flex-col space-y-4 md:space-y-6 animate-in fade-in duration-300">
                ...
                <div onClick={() => setActiveModal('spending_details')} className="bg-white rounded-xl shadow p-5 md:p-6 relative cursor-pointer hover:shadow-md transition group border border-transparent hover:border-[#0071ce]">
